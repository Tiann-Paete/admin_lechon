...
async function handleAddCustomer(req, res) {
  ...
  try {
    const [result] = await db.query(
      'INSERT INTO acustomer (name, address, contactNumber) VALUES (?, ?, ?)',
      [name, address, contactNumber]
    );
    res.status(201).json({ id: result.insertId, name, address, contactNumber });
    console.log('Customer added successfully:', { id: result.insertId, name, address, contactNumber });
  } catch (error) {
...
async function handleUpdateCustomer(req, res, id) {
  ...
  try {
    ...
    res.status(200).json({ id, name, address, contactNumber });
    console.log('Customer updated successfully:', { id, name, address, contactNumber });
  } catch (error) {
